...
rm(list=setdiff(ls(), c("absData","absData1","thresh","clinData","annData","DATA.DIR","RES.DIR","totalCountData")))
...
qqplot <- ggplot(snps2genesP, aes(sample = logp)) +
  geom_qq(shape = 1, size = 0.5, distribution = qchisq, dparams = list(df = 1)) +
