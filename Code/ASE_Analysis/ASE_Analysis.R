...
homtest$ZYG <- as.factor(homtest$ZYG)

# Create ROC and calculate threshold using Youden's J statistic
roc <- roc(homtest$ZYG,homtest$ASE,plot=T)
thresh <- as.numeric(coords(roc, x="best", ret="threshold", 
                            best.method="youden"))
...
# Keep only SIDs present in absData
clinData <- clinData[clinData$SID %in% colnames(absData),]
...
if (geno == TRUE) {
  ...
  # Get top 10 known Disease-associated genes and classify each p-value
  setwd(DATA.DIR)
  genesOI <- read.table(genes_of_interest, header = T, sep = "\t")
  
  snps2genesP$top <- "All other genes"
  snps2genesP$top[snps2genesP$ensembl_gene_id %in% genesOI$ensembl_gene_id] <- "Robust genes"
  ...
  dev.off()
    ...
    apm <- pivot_longer(apm, c(1:(ncol(apm)-1)),names_to = "SID")
    colnames(apm)[3] <- "pvalue"
    apm <- na.omit(apm)
    apm <- apm[!apm$pvalue==0,]
    snps2genesP <- full_join(apm1,snp2geneData)
    snps2genesP <- snps2genesP[!is.na(snps2genesP$pvalue),]
...
diffAseAcross <- function(asedata, clinicaldata, groupVar) {
  ...
  kwVals <- apply(asedata, 1, function(x) {
    ...
    kw <- NA
    try ({
      kw <- kruskal.test(testData)$p.value
    }, silent = TRUE
    )
    return(kw)
  })
...
ase.qtl.enrich(inputData = snps2genesPacross,
               sqtl = gtexsqtl,
               eqtl = gtexeqtl)

# Make boxplot for most significant SNP hit in across phenogroups comparison
...
tiff(filename=paste0("Boxplot_",snpBoxplotAcross,"_across.tif"), 
...
setwd(DATA.DIR)
#save.image("ASE_DCM_18052022.RData")
load("ASE_DCM_18052022.RData")
